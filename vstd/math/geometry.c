...
void geometry_deduplicate_vertices(u32 vertex_count, vertex_3d *vertices,
    ...
    vertex_3d *unique_verts =
        vallocate(sizeof(vertex_3d) * vertex_count, MEMORY_TAG_ARRAY);
    *out_vertex_count = 0;
    ...
    *out_vertices =
        vallocate(sizeof(vertex_3d) * (*out_vertex_count), MEMORY_TAG_ARRAY);
    // Copy over unique
    vcopy_memory(*out_vertices, unique_verts,
                 sizeof(vertex_3d) * (*out_vertex_count));
    // Destroy temp array
    vfree(unique_verts, sizeof(vertex_3d) * vertex_count, MEMORY_TAG_ARRAY);

    VDEBUG("geometry_deduplicate_vertices: removed %d vertices, orig/now %d/%d.",
...
void generate_uvs_from_image_coords(u32 img_width, u32 img_height, u32 px_x, u32 px_y, f32 *out_tx, f32 *out_ty) {
    vassert_DEBUG(out_tx);
    vassert_DEBUG(out_ty);
...
void generate_quad_2d(const char *name, f32 width, f32 height, f32 tx_min, f32 tx_max, f32 ty_min, f32 ty_max, geometry_config *out_config) {
    if (out_config) {
        vzero_memory(out_config, sizeof(geometry_config));
        out_config->vertex_size = sizeof(vertex_2d);
        out_config->vertex_count = 4;
        out_config->vertices = vallocate(out_config->vertex_size * out_config->vertex_count, MEMORY_TAG_ARRAY);
        out_config->index_size = sizeof(u32);
        out_config->index_count = 6;
        out_config->indices = vallocate(out_config->index_size * out_config->index_count, MEMORY_TAG_ARRAY);
        ...
        vcopy_memory(out_config->vertices, uiverts, out_config->vertex_size * out_config->vertex_count);

        // Indices - counter-clockwise
        u32 uiindices[6] = {2, 1, 0, 3, 0, 1};
        vcopy_memory(out_config->indices, uiindices, out_config->index_size * out_config->index_count);
    }
}
