#ifndef __vinput_types__
#define __vinput_types__
/**
 * @brief Represents mouse buttons.
 */
typedef enum VMOUSE_KEY {
    /** @brief The left mouse button */
    VMOUSE_VKEY_M0,
    /** @brief The right mouse button */
    VMOUSE_VKEY_M1,
    /** @brief The middle mouse button (typically the wheel) */
    VMOUSE_VKEY_M2,
    /** @brief Maximum number of mouse buttons */
    VMOUSE_VKEY_MAX
} VMOUSE_KEY;

/**
 * @brief Represents available keyboard keys.
 */
typedef enum VKEY {
    /** @brief The backspace key. */
    VKEY_BACKSPACE = 0x08,
    /** @brief The enter key. */
    VKEY_ENTER = 0x0D,
    /** @brief The tab key. */
    VKEY_TAB = 0x09,
    /** @brief The shift key. */
    VKEY_SHIFT = 0x10,
    /** @brief The Control/Ctrl key. */
    VKEY_CONTROL = 0x11,

    /** @brief The pause key. */
    VKEY_PAUSE = 0x13,
    /** @brief The Caps Lock key. */
    VKEY_CAPITAL = 0x14,

    /** @brief The Escape key. */
    VKEY_ESCAPE = 0x1B,

    VKEY_CONVERT = 0x1C,
    VKEY_NONCONVERT = 0x1D,
    VKEY_ACCEPT = 0x1E,
    VKEY_MODECHANGE = 0x1F,

    /** @brief The spacebar key. */
    VKEY_SPACE = 0x20,
    /** @brief The page up key. */
    VKEY_PAGEUP = 0x21,
    /** @brief The page down key. */
    VKEY_PAGEDOWN = 0x22,
    /** @brief The end key. */
    VKEY_END = 0x23,
    /** @brief The home key. */
    VKEY_HOME = 0x24,
    /** @brief The left arrow key. */
    VKEY_LEFT = 0x25,
    /** @brief The up arrow key. */
    VKEY_UP = 0x26,
    /** @brief The right arrow key. */
    VKEY_RIGHT = 0x27,
    /** @brief The down arrow key. */
    VKEY_DOWN = 0x28,
    VKEY_SELECT = 0x29,
    VKEY_PRINT = 0x2A,
    VKEY_EXECUTE = 0x2B,
    /** @brief The Print Screen key. */
    VKEY_PRINTSCREEN = 0x2C,
    /** @brief The insert key. */
    VKEY_INSERT = 0x2D,
    /** @brief The delete key. */
    VKEY_DELETE = 0x2E,
    VKEY_HELP = 0x2F,

    /** @brief The 0 key */
    VKEY_0 = 0x30,
    /** @brief The 1 key */
    VKEY_1 = 0x31,
    /** @brief The 2 key */
    VKEY_2 = 0x32,
    /** @brief The 3 key */
    VKEY_3 = 0x33,
    /** @brief The 4 key */
    VKEY_4 = 0x34,
    /** @brief The 5 key */
    VKEY_5 = 0x35,
    /** @brief The 6 key */
    VKEY_6 = 0x36,
    /** @brief The 7 key */
    VKEY_7 = 0x37,
    /** @brief The 8 key */
    VKEY_8 = 0x38,
    /** @brief The 9 key */
    VKEY_9 = 0x39,

    /** @brief The A key. */
    VKEY_A = 0x41,
    /** @brief The B key. */
    VKEY_B = 0x42,
    /** @brief The C key. */
    VKEY_C = 0x43,
    /** @brief The D key. */
    VKEY_D = 0x44,
    /** @brief The E key. */
    VKEY_E = 0x45,
    /** @brief The F key. */
    VKEY_F = 0x46,
    /** @brief The G key. */
    VKEY_G = 0x47,
    /** @brief The H key. */
    VKEY_H = 0x48,
    /** @brief The I key. */
    VKEY_I = 0x49,
    /** @brief The J key. */
    VKEY_J = 0x4A,
    /** @brief The K key. */
    VKEY_K = 0x4B,
    /** @brief The L key. */
    VKEY_L = 0x4C,
    /** @brief The M key. */
    VKEY_M = 0x4D,
    /** @brief The N key. */
    VKEY_N = 0x4E,
    /** @brief The O key. */
    VKEY_O = 0x4F,
    /** @brief The P key. */
    VKEY_P = 0x50,
    /** @brief The Q key. */
    VKEY_Q = 0x51,
    /** @brief The R key. */
    VKEY_R = 0x52,
    /** @brief The S key. */
    VKEY_S = 0x53,
    /** @brief The T key. */
    VKEY_T = 0x54,
    /** @brief The U key. */
    VKEY_U = 0x55,
    /** @brief The V key. */
    VKEY_V = 0x56,
    /** @brief The W key. */
    VKEY_W = 0x57,
    /** @brief The X key. */
    VKEY_X = 0x58,
    /** @brief The Y key. */
    VKEY_Y = 0x59,
    /** @brief The Z key. */
    VKEY_Z = 0x5A,

    /** @brief The left Windows/Super key. */
    VKEY_LSUPER = 0x5B,
    /** @brief The right Windows/Super key. */
    VKEY_RSUPER = 0x5C,
    /** @brief The applicatons key. */
    VKEY_APPS = 0x5D,

    /** @brief The sleep key. */
    VKEY_SLEEP = 0x5F,

    /** @brief The numberpad 0 key. */
    VKEY_NUMPAD0 = 0x60,
    /** @brief The numberpad 1 key. */
    VKEY_NUMPAD1 = 0x61,
    /** @brief The numberpad 2 key. */
    VKEY_NUMPAD2 = 0x62,
    /** @brief The numberpad 3 key. */
    VKEY_NUMPAD3 = 0x63,
    /** @brief The numberpad 4 key. */
    VKEY_NUMPAD4 = 0x64,
    /** @brief The numberpad 5 key. */
    VKEY_NUMPAD5 = 0x65,
    /** @brief The numberpad 6 key. */
    VKEY_NUMPAD6 = 0x66,
    /** @brief The numberpad 7 key. */
    VKEY_NUMPAD7 = 0x67,
    /** @brief The numberpad 8 key. */
    VKEY_NUMPAD8 = 0x68,
    /** @brief The numberpad 9 key. */
    VKEY_NUMPAD9 = 0x69,
    /** @brief The numberpad multiply key. */
    VKEY_MULTIPLY = 0x6A,
    /** @brief The numberpad add key. */
    VKEY_ADD = 0x6B,
    /** @brief The numberpad separator key. */
    VKEY_SEPARATOR = 0x6C,
    /** @brief The numberpad subtract key. */
    VKEY_SUBTRACT = 0x6D,
    /** @brief The numberpad decimal key. */
    VKEY_DECIMAL = 0x6E,
    /** @brief The numberpad divide key. */
    VKEY_DIVIDE = 0x6F,

    /** @brief The F1 key. */
    VKEY_F1 = 0x70,
    /** @brief The F2 key. */
    VKEY_F2 = 0x71,
    /** @brief The F3 key. */
    VKEY_F3 = 0x72,
    /** @brief The F4 key. */
    VKEY_F4 = 0x73,
    /** @brief The F5 key. */
    VKEY_F5 = 0x74,
    /** @brief The F6 key. */
    VKEY_F6 = 0x75,
    /** @brief The F7 key. */
    VKEY_F7 = 0x76,
    /** @brief The F8 key. */
    VKEY_F8 = 0x77,
    /** @brief The F9 key. */
    VKEY_F9 = 0x78,
    /** @brief The F10 key. */
    VKEY_F10 = 0x79,
    /** @brief The F11 key. */
    VKEY_F11 = 0x7A,
    /** @brief The F12 key. */
    VKEY_F12 = 0x7B,
    /** @brief The F13 key. */
    VKEY_F13 = 0x7C,
    /** @brief The F14 key. */
    VKEY_F14 = 0x7D,
    /** @brief The F15 key. */
    VKEY_F15 = 0x7E,
    /** @brief The F16 key. */
    VKEY_F16 = 0x7F,
    /** @brief The F17 key. */
    VKEY_F17 = 0x80,
    /** @brief The F18 key. */
    VKEY_F18 = 0x81,
    /** @brief The F19 key. */
    VKEY_F19 = 0x82,
    /** @brief The F20 key. */
    VKEY_F20 = 0x83,
    /** @brief The F21 key. */
    VKEY_F21 = 0x84,
    /** @brief The F22 key. */
    VKEY_F22 = 0x85,
    /** @brief The F23 key. */
    VKEY_F23 = 0x86,
    /** @brief The F24 key. */
    VKEY_F24 = 0x87,

    /** @brief The number lock key. */
    VKEY_NUMLOCK = 0x90,

    /** @brief The scroll lock key. */
    VKEY_SCROLL = 0x91,

    /** @brief The numberpad equal key. */
    VKEY_NUMPAD_EQUAL = 0x92,

    /** @brief The left shift key. */
    VKEY_LSHIFT = 0xA0,
    /** @brief The right shift key. */
    VKEY_RSHIFT = 0xA1,
    /** @brief The left control key. */
    VKEY_LCONTROL = 0xA2,
    /** @brief The right control key. */
    VKEY_RCONTROL = 0xA3,
    /** @brief The left alt key. */
    VKEY_LALT = 0xA4,
    /** @brief The right alt key. */
    VKEY_RALT = 0xA5,

    /** @brief The semicolon key. */
    VKEY_SEMICOLON = 0x3B,

    /** @brief The apostrophe/single-quote key */
    VKEY_APOSTROPHE = 0xDE,
    /** @brief An alias for VKEY_APOSTROPHE, apostrophe/single-quote key */
    VKEY_QUOTE = VKEY_APOSTROPHE,
    /** @brief The equal/plus key. */
    VKEY_EQUAL = 0xBB,
    /** @brief The comma key. */
    VKEY_COMMA = 0xBC,
    /** @brief The minus key. */
    VKEY_MINUS = 0xBD,
    /** @brief The period key. */
    VKEY_PERIOD = 0xBE,
    /** @brief The slash key. */
    VKEY_SLASH = 0xBF,

    /** @brief The grave key. */
    VKEY_GRAVE = 0xC0,

    /** @brief The left (square) bracket key e.g. [{ */
    VKEY_LBRACKET = 0xDB,
    /** @brief The pipe/backslash key */
    VKEY_PIPE = 0xDC,
    /** @brief An alias for the pipe/backslash key */
    VKEY_BACKSLASH = VKEY_PIPE,
    /** @brief The right (square) bracket key e.g. ]} */
    VKEY_RBRACKET = 0xDD,

    KEYS_MAX_KEYS = 0xFF
} VKEY;

#endif