...
#define VSIZE_STORAGE sizeof(u32)
...
b8 dynamic_allocator_create(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator* out_allocator) {
    ...
    *memory_requirement = freelist_requirement + sizeof(dynamic_allocator_state) + total_size;
    ...
    state->freelist_block = (char*)(out_allocator->memory + sizeof(dynamic_allocator_state));
    state->memory_block = (char*)(state->freelist_block + freelist_requirement);
...
void* dynamic_allocator_allocate_aligned(dynamic_allocator* allocator, u64 size, u16 alignment) {
    if (allocator && size && alignment) {
        ...
        u64 storage_size = VSIZE_STORAGE;
        ...
        VASSERT_MSG(required_size < 4294967295U, "dynamic_allocator_allocate_aligned called with required size > 4 GiB. Don't do that.");
        ...
        if (freelist_allocate_block(&state->list, required_size, &base_offset)) {
            ...
            */
            ...
            u64 aligned_block_offset = get_aligned((u64)ptr + VSIZE_STORAGE, alignment);
            // Store the size just before the user data block
            u32* block_size = (u32*)(aligned_block_offset - VSIZE_STORAGE);
            *block_size = (u32)size;
...
b8 dynamic_allocator_free_aligned(dynamic_allocator* allocator, void* block) {
    ...
    u32* block_size = (u32*)((u64)block - VSIZE_STORAGE);
    alloc_header* header = (alloc_header*)((u64)block + *block_size);
    u64 required_size = header->alignment + sizeof(alloc_header) + VSIZE_STORAGE + *block_size;
...
b8 dynamic_allocator_get_size_alignment(void* block, u64* out_size, u16* out_alignment) {
    // Get the header.
    *out_size = *(u32*)((u64)block - VSIZE_STORAGE);
...
u64 dynamic_allocator_header_size(void) {
    ...
    return sizeof(alloc_header) + VSIZE_STORAGE;
}
