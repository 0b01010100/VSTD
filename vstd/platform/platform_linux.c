...
static vwindow* window_from_handle(xcb_window_t window);
...
b8 platform_system_startup(u64* memory_requirement, struct platform_state* state, platform_system_config* config) {
    *memory_requirement = sizeof(platform_state);
    ...
    for (i32 s = 0; s < state_ptr->screen_count; ++s) {
        ...
        VINFO("Monitor '%s' has a DPI of %.2f for a device pixelratio of %0.2f", it.index, x_dpi, x_dpi / 96.0f);
    ...
    state_ptr->windows = darray_create(vwindow*);
...
b8 platform_window_create(const vwindow_config* config, struct vwindow* window, b8 show_immediately) {
    if (!window) {
        return false;
    }
    ...
    window->platform_state = vallocate(sizeof(vwindow_platform_state), MEMORY_TAG_UNKNOWN);
                        ...
                        for (i32 m = 0; m < resources->nmode; ++m) {
                            ...
                            if (mode_info->id == crtc_info->mode) {
                                ...
                                if (XRRGetCrtcTransform(state_ptr->display, op_info->crtc, &attr) && attr) {
                                    ...
                                    // scale = 1.0f / scale;
                                    VTRACE("Scale before: %.2f", scale);
                                    scale = range_convert_f32(scale, 0.0f, 1.0f, 1.0f, 2.0f);
                                    VTRACE("Scale after: %.2f", scale);
                                    ...
                                    VTRACE("device_pixel_ratio x/y: %.2f, %.2f", dpi_x / 96.0f, dpi_y / 96.0f);

                                    VTRACE("Scale x/y: %i/%i, actual w/h: %i/%i", scale_w >> 16, scale_h >> 16, actual_w, actual_h);
                                    XFree(attr);
                                }
                            }
                        }
    ...
    if (resource_string) {
        VTRACE("Entire DB: '%s'", resource_string);
        if (XrmGetResource(db, "Xft.dpi", "String", &type, &value) == true) {
            if (value.addr) {
                if (!string_to_f32(value.addr, &dpi)) {
                    VERROR("Unable to parse DPI from Xft.dpi");
                }
            }
        }
    }
    */
    ...
    if (config->title) {
        ...
        window->title = string_duplicate("VIRTUNITY Game Engine Window");
    }
...
void platform_window_destroy(struct vwindow* window) {
    if (window) {
        u32 len = darray_length(state_ptr->windows);
        for (u32 i = 0; i < len; ++i) {
            if (state_ptr->windows[i] == window) {
                VTRACE("Destroying window...");
...
b8 platform_window_show(struct vwindow* window) {
    if (!window) {
        return false;
    }
    // Show the window
...
b8 platform_window_hide(struct vwindow* window) {
    if (!window) {
        return false;
    }
...
const char* platform_window_title_get(const struct vwindow* window) {
...
b8 platform_window_title_set(struct vwindow* window, const char* title) {
    if (!window) {
        return false;
    }
    ...
    if (state_ptr) {
        ...
        while ((event = xcb_poll_for_event(state_ptr->handle.connection))) {
            ...
            case XCB_CONFIGURE_NOTIFY: {
                ...
                vwindow* w = window_from_handle(configure_event->window);
                if (!w) {
                    VERROR("Recieved a window resize event for a non-registered window!");
                    return 0;
                }
...
i32 platform_get_processor_count(void) {
    ...
    VINFO("%i processor cores detected, %i cores available.", processor_count, processors_available);
...
void platform_get_handle_info(u64* out_size, void* memory) {
    *out_size = sizeof(linux_handle_info);
    ...
    vcopy_memory(memory, &state_ptr->handle, *out_size);
}

f32 platform_device_pixel_ratio(const struct vwindow* window) {
...
platform_error_code platform_copy_file(const char* source, const char* dest, b8 overwrite_if_exists) {
    ...
    if (source_fd == -1) {
        if (errno == ENOENT) {
            VERROR("Source file does not exist: %s", source);
        }
    ...
    if (dest_fd == -1) {
        if (errno == ENOENT) {
            VERROR("Destination file could not be created: %s", dest);
        }
...
close_handles:
    if (source_fd != -1) {
        ...
        if (result != 0) {
            VERROR("Error closing source file: %s", source);
        }
    }
    if (dest_fd != -1) {
        result = close(dest_fd);
        if (result != 0) {
            VERROR("Error closing destination file: %s", source);
        }
    }
...
static b8 unregister_watch(u32 watch_id) {
    ...
    vfree((void*)w->file_path, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    w->file_path = 0;
    vzero_memory(&w->last_write_time, sizeof(long));
        ...
        if (f->id != INVALID_ID) {
            ...
            if (result != 0) {
                if (errno == ENOENT) {
                    ...
                    if (state_ptr->watcher_deleted_callback) {
                        ...
                        VWARN("Watcher file was deleted but no handler callback was set. Make sure to call platform_register_watcher_deleted_callback()");
                    }
                    ...
                    VWARN("Some other error occurred on file watch id %d", f->id);
                }
            ...
            if (info.st_mtime - f->last_write_time != 0) {
                ...
                if (state_ptr->watcher_written_callback) {
                    ...
                    VWARN("Watcher file was deleted but no handler callback was set. Make sure to call platform_register_watcher_written_callback()");
                }
            }
        }
    }
