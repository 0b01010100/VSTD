...
static void handle_resize(vwindow* w, u16 width, u16 height);
...
- (void)mouseMoved:(NSEvent*)event {
    ...
    vwindow* w = ((WindowDelegate*)event.window.delegate)->VIRTUNITY_window;
    vwindow_platform_state* ps = w->platform_state;
...
- (BOOL)windowShouldClose:(NSWindow*)sender {
    ...
    if (state_ptr->window_closed_callback) {
        vwindow* w = ((WindowDelegate*)sender.delegate)->VIRTUNITY_window;
        if (!w) {
            VERROR("Recieved a window close event for a non-registered window!");
            return NO;
        }
...
- (void)windowDidChangeScreen:(NSNotification*)notification {
    ...
    vwindow* w = ((WindowDelegate*)window.delegate)->VIRTUNITY_window;
    vwindow_platform_state* ps = w->platform_state;
...
b8 platform_system_startup(u64* memory_requirement, platform_state* state, platform_system_config* config) {
    *memory_requirement = sizeof(platform_state);
    ...
    state->windows = darray_create(vwindow*);
    ...
    @autoreleasepool {
        ...
        if (!state_ptr->app_delegate) {
            VERROR("Failed to create application delegate")
            return false;
        }
...
b8 platform_window_create(const vwindow_config* config, struct vwindow* window, b8 show_immediately) {
    ...
    ps->handle = [[NSWindow alloc]
        initWithContentRect:NSMakeRect(client_x, client_y, client_width, client_height)
                  styleMask:NSWindowStyleMasVMINiaturizable | NSWindowStyleMaskTitled | NSWindowStyleMaskClosable | NSWindowStyleMaskResizable
                    backing:NSBackingStoreBuffered
                      defer:NO];
    if (!ps->handle) {
        VERROR("Failed to create window");
        return false;
    }
    ...
    if (!ps->layer) {
        VERROR("Failed to create layer for view");
    }
    ...
    ps->layer.contentsScale = ps->view.window.backingScaleFactor;
    VDEBUG("contentScale: %f", ps->layer.contentsScale);
    // Save off the device pixel ratio.
    ps->device_pixel_ratio = ps->layer.contentsScale;
...
static void handle_modifier_keys(u32 ns_keycode, u32 modifier_flags) {
    ...
    VTRACE("modifier flags keycode: %u", ns_keycode);
