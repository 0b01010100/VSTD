...
b8 filesystem_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle) {
    ...
    if ((mode & FILE_MODE_READ) != 0 && (mode & FILE_MODE_WRITE) != 0) {
        ...
        VERROR("Invalid mode passed while trying to open file: '%s'", path);
        return false;
    }
    ...
    if (!file) {
        VERROR("Error opening file: '%s'", path);
        return false;
    }
...
const char* filesystem_read_entire_text_file(const char* filepath) {
    ...
    char* buf = vallocate(size + 1, MEMORY_TAG_STRING);
    ...
    if (bytes_read < size) {
        ...
        vfree(buf, size, MEMORY_TAG_STRING);
        return copy;
    }
    return buf;
}
