...
#if _WIN32 // VPLATFORM_WINDOWS // FIXME: macro doesn't highlight correctly in vscode
...
b8 platform_system_startup(u64* memory_requirement, platform_state* state, platform_system_config* config) {
    *memory_requirement = sizeof(platform_state);
    ...
    wc.lpszClassName = L"VIRTUNITY_window_class";
    wc.lpszMenuName = 0;
    ...
    if (!RegisterClassExW(&wc)) {
        ...
        if (size) {
            ...
            VFATAL("Window registration failed!");
        }
...
b8 platform_window_create(const vwindow_config* config, struct vwindow* window, b8 show_immediately) {
    ...
    window->platform_state->hwnd = CreateWindowExW(
        window_ex_style, L"VIRTUNITY_window_class", wtitle,
...
b8 platform_dynamic_library_load(const char* name, dynamic_library* out_library) {
    ...
    vzero_memory(filename, sizeof(char) * MAX_PATH);
...
static b8 unregister_watch(u32 watch_id) {
    ...
    w->file_path = 0;
    vzero_memory(&w->last_write_time, sizeof(FILETIME));
...
static vwindow* window_from_handle(HWND hwnd) {
    u32 len = darray_length(state_ptr->windows);
    for (u32 i = 0; i < len; ++i) {
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param) {
    ...
    case WM_CLOSE: {
        if (state_ptr->window_closed_callback) {
            vwindow* w = window_from_handle(hwnd);
            if (!w) {
    ...
    case WM_DPICHANGED:
        ...
        VINFO("Display device pixel ratio is: %.2f", w->platform_state->device_pixel_ratio);
...
static LPCWSTR cstr_to_wcstr(const char* str) {
    ...
    wchar_t* wstr = vallocate(sizeof(wchar_t) * (len), MEMORY_TAG_STRING);
    if (!wstr) {
        return 0;
    }
    if (MultiByteToWideChar(CP_UTF8, 0, str, -1, wstr, len) == 0) {
        vfree((wchar_t*)wstr, sizeof(wchar_t) * (len), MEMORY_TAG_STRING);
        return 0;
    }
    return wstr;
}
...
static void wcstr_free(LPCWSTR wstr) {
    if (wstr) {
        ...
        vfree((WCHAR*)wstr, sizeof(WCHAR) * len, MEMORY_TAG_STRING);
    }
}
...
static const char* wcstr_to_cstr(LPCWSTR wstr) {
    ...
    char* str = vallocate(sizeof(WCHAR) * length, MEMORY_TAG_STRING);
    if (!str) {
        return 0;
    }
    if (WideCharToMultiByte(CP_UTF8, 0, wstr, -1, str, length, NULL, NULL) == 0) {
        vfree((char*)str, sizeof(char) * (length + 1), MEMORY_TAG_STRING);
        return 0;
    }
